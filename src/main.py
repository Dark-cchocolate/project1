...
def run_bstar_insertion_experiment(records, d):
    tree = BStarTree(d=d)
    ...
    print(f"Insertion time: {insertion_time:.6f} seconds")
    print(f"Split count: {tree.split_count}")
    print(f"Redistribution count: {tree.redistribution_count}")
    print(f"2-to-3 split count: {tree.two_to_three_split_count}")
    print(f"Height: {tree.get_height()}")
    print(f"Node count: {tree.count_nodes()}")
    print(f"Node utilization: {tree.get_node_utilization():.6f}")

    violations = tree.validate_max_keys()
    print(f"Max-key violations: {len(violations)}")
